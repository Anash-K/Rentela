...
import { processPaymentCompletedMessage } from "../services/invoice.processor.js";

const logger = pino({ name: "invoice-kafka-consumer" });
  ...
  for (let attempt = 1; attempt <= PAYMENT_COMPLETED_MAX_PROCESS_ATTEMPTS; attempt++) {
    try {
      ...
      log.warn(
        ...
        "invoice consume attempt failed",
      );
...
export async function startInvoicePaymentCompletedConsumer() {
  ...
  logger.info(
    ...
    "invoice-service subscribed to payment.completed",
  );
